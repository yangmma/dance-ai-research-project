...
down_weighted = [20.0, 5.0, 5.0, 5.0, 5.0, 1.0, 1.0, 0.5, 0.5]
# SMPL_JOINT_NAMES = [
#     "root",     
#     "lhip", "rhip", "belly",    
#     "lknee", "rknee", "spine",    
...
def _loss_fn(x_target, x_pred, weighted=False):
    ...
    if not weighted:
        ...
        # print(tt, flush=True)
        # assert(tt == 15)
        return t.mean(t.abs(x_pred.view(n, tt, 15, c//15) - x_target.view(n, tt, 15, c//15)) * t.tensor(up_weighted).cuda().view(1, 1, 15, 1))
...
class VQVAEmix(nn.Module):
    def __init__(self, hps, input_dim=72, output_dim=72):
        ...
        decoder = lambda level: Decoder(x_channels, emb_width, level + 1,
                                        downs_t[:level+1], strides_t[:level+1], **_block_kwargs(level))
        # print('x_out_channels ', x_out_channels, flush=True)
    ...
    def _decode(self, zs, start_level=0, end_level=None):
        ...
        decoder, x_quantised = self.decoders_rot[start_level], xs_quantised[0:1]
